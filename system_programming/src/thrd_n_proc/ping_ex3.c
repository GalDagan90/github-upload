/*********************************************************
 *	Author: Gal Dagan
 *	Reviewer: Navot Yaari
 *	Date: 06.10.2021.
 *
 *	Description: signals ping pong ex3 ping - parent
 *
 *	Infinity Labs OL108
 *********************************************************/
#define _POSIX_C_SOURCE 199309L

#include <stdio.h>      /* printf() */
#include <signal.h>     /* sigaction(), kill(), SIGUSR1, SIGUSR2, sigemptyset()
                           sigaddset(), sigprocmask(), sigwaitinfo()
                           sigsuspend(), struct sigaction, sigset_t, siginfo_t */
#include <sys/types.h>  /* pid_t */
#include <unistd.h>     /* getpid() */
#include <string.h>     /* memset() */

#define UNUSED(x) ((void)(x))
#define NUM 10000

static pid_t pid = 0;
static sig_atomic_t sig_counter = 0;

/************************************************************
*                function declaration                       *
*************************************************************/  
static void PingHandler(int signum);
static void WaitForSignal(sigset_t *oldmask);
static void SetBlockedSignal(sigset_t *oldmask, int signum);
static pid_t GetPidFromSignal(int signum);
static void SetSignalHandler(void (*handler_func)(int), int signum);

/***********************************************************/

int main(void)
{
    sigset_t oldmask;

    SetSignalHandler(PingHandler, SIGUSR2);
    SetBlockedSignal(&oldmask, SIGUSR2);

    pid = GetPidFromSignal(SIGUSR2);
    kill(pid, SIGUSR1);

    while (NUM > sig_counter)
    {
        WaitForSignal(&oldmask);
        kill(pid, SIGUSR1);
    }

    printf("%d\n", sig_counter);

    return (0);
}

/************* Implementation *********************************/
static void PingHandler(int signum)
{
    UNUSED(signum);
    ++sig_counter;
}

static void WaitForSignal(sigset_t *oldmask)
{
    sig_atomic_t counter = sig_counter;

    sigfillset(oldmask);
    sigdelset(oldmask, SIGUSR2);
    
    while (counter == sig_counter)
    {
        sigsuspend(oldmask);
    }
}

static void SetBlockedSignal(sigset_t *oldmask, int signum)
{
    sigset_t mask;

    sigemptyset(&mask);
    sigaddset(&mask, signum);
    sigprocmask(SIG_BLOCK, &mask, oldmask);
}

static pid_t GetPidFromSignal(int signum)
{
    sigset_t mask;
    siginfo_t info_pid;

    sigemptyset(&mask);
    sigaddset(&mask, signum);
    sigwaitinfo(&mask, &info_pid);

    return (info_pid.si_pid);
}

static void SetSignalHandler(void (*handler_func)(int), int signum)
{
    struct sigaction sa = {0};

    memset(&sa, 0, sizeof(struct sigaction));
    sa.sa_handler = handler_func;
    sigaction(signum, &sa, NULL);
}